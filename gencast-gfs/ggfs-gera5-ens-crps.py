...
OUT_FIG = "/home/mihir.more/myenv/ggfs-gera5_ens_vs_imerg_crps.png"

# Use mm (True) or meters (False) for CRPS units
CRPS_IN_MM = True


def load_gencast_tp_inc(run_dir: str, pattern: str):
...
# --------------------
# 1) Load GenCast forecasts (SST-injected, ERA5-initialized)
# --------------------
gc_sst_tp_inc = load_gencast_tp_inc(gencast_sst_dir, pattern="gc_gfs_*.grib2")
gc_era5_tp_inc = load_gencast_tp_inc(gencast_era5_dir, pattern="gencast-025-oper-*.grib")

# Align ERA5 run to 06/18 UTC bins (shift +6h so it matches IMERG/GFS 00/12 bins)
gc_era5_tp_inc = gc_era5_tp_inc.assign_coords(valid_time=gc_era5_tp_inc.valid_time + np.timedelta64(6, "h"))
...
# --------------------
target_lat = gc_sst_tp_inc["latitude"]
target_lon = gc_sst_tp_inc["longitude"]
...
for k, fc in forecasts.items():
    ...
    # ensure non-empty and dense
    if aligned[k].sizes.get("valid_time", 0) == 0:
