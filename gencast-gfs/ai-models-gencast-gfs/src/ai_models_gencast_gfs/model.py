"""
GenCast ai-models-gfs plugin built from ai-models-gencast.

This wraps the upstream GenCast implementation (GraphCast repo) but uses the
`ai_models_gfs.model.Model` base so it plugs into the GFS pipeline.
"""
...
# Optional heavy deps: import lazily so the module can be imported without JAX.
_DEPS_ERROR: Exception | None = None
try:  # pragma: no cover - exercised at runtime
    import haiku as hk
    import jax
    from graphcast import checkpoint, data_utils, gencast, nan_cleaning, normalization, rollout, xarray_jax
except ModuleNotFoundError as exc:  # pragma: no cover
    _DEPS_ERROR = exc
    hk = jax = checkpoint = data_utils = gencast = nan_cleaning = normalization = rollout = xarray_jax = None

HAS_DEPS = _DEPS_ERROR is None
...
class GenCast(Model):
    ...
    @property
    def constant_fields(self):  # pragma: no cover - used by retrieval filters
        return {"z", "lsm"}

    def _require_deps(self) -> None:
        if _DEPS_ERROR is not None:
            msg = "You need to install Graphcast/GenCast (JAX + graphcast) to use this model."
            LOG.error(msg)
            raise ModuleNotFoundError(f"{msg}\n{_DEPS_ERROR}")

    def _with_configs(self, fn: Callable):
    ...
    def download_assets(self, **kwargs: Any) -> None:  # pragma: no cover - thin wrapper
    ...
    def patch_retrieve_request(self, r):  # pragma: no cover
    ...
    def parse_model_args(self, args):  # pragma: no cover
        import argparse

        parser = argparse.ArgumentParser("ai-models-gfs gencast")
        parser.add_argument(
...
def model(model_version: str = "latest", **kwargs: Any) -> GenCast:
    models = {
        "0.25": GenCast0p25deg,
    ...
    if model_version not in models:
        LOG.error("Model version %s not found, available: %s", model_version, list(models.keys()))
